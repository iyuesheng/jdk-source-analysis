...
/**
 * ArrayList是实现List接口的可自动扩容的数组。
 * 实现了所有的list操作，允许所有的元素，包括null值。
 * 除了实现了List接口，该类提供了一些操作数组size的方法，
 * 用于内部存储list。
 *
 * ArrayList大致和Vector相同，除了ArrayList是非同步的。
 *
 * size isEmpty get set iterator 和 listIterator 方法时间复杂度是O(1)，常量时间。
 * 其他方法是O(n)，线性时间。
 * 常量因子比LinkedList低。
 * 
 * 每一个ArrayList实例都有一个capacity（容量）。
 * capacity是用于存储列表中元素的数组的大小。
 * capacity至少和列表的大小一样大。
 *
 * 当元素被添加到ArrayList中的时候，capacity自动扩大。
 * 
 * 扩容策略可以平摊给add 方法，这样add方法还是O(1)时间复杂度。
 *
 * 应用可以用ensureCapacity方法提前进行扩容。
 * 这可能减少自动扩容的次数。
 * 
 * ArrayList是非同步的。
 * 
 * 如果多个线程同时访问ArrayList的实例，并且至少一个线程会修改，
 * 必须在外部保证ArrayList的同步。
 * 修改包括添加删除扩容等操作，仅仅设置值不包括。
 *
 * 这种场景可以用其他的一些封装好的同步的list。 
        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
    ...
    /**
     * 用于提供默认大小的实例的共享空数组实例
     * EMPTY_ELEMENTDATA 第一次添加的时候，会扩容到 容量 = 1
     * DEFAULTCAPACITY_EMPTY_ELEMENTDATA 第一次添加元素的时候，会扩容到 容量 = 10
     */
     ...
     * 存储ArrayList元素的数组缓冲区
     * ArrayList的容量，是数组的长度
     * 所有空ArrayList elementData == DEFAULTCAPACITY_EMPTY_ELEMENTDATA
     * 当添加第一个元素的时候，将会扩容到DEFAULT_CAPACITY
     */
    transient Object[] elementData; // non-private to simplify nested class access

    /**
     * ArrayList的大小（ArrayList中元素的数量）
     */
    private int size;
     ...
     * @throws  如果初始容量非法就抛出IllegalArgumentException
     */
    ...
    public ArrayList(Collection<? extends E> c) {
        ...
        if ((size = elementData.length) != 0) {
            // c.toArray 可能不正确的，不返回 Object[] (see 6260652)
    ...
    /**
     * ArrayList实例添加元素之前，确保该实例可以容纳minCapacity的元素
     *
     ...
     * 尝试分配更大的size，可能导致OutOfMemoryError
     */
